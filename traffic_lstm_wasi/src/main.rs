...
fn main() -> TractResult<()> {
    ...
    const TARGET_SENSOR_INDEX: usize = 3; // uzimamo podatke 3. senzora za test kako bi se isprobalo na drugim mjerenjima
    ...
    // ČITANJE PODATAKA
    println!("[INFO] Čitam podatke iz '{}' (Senzor #{})...", CSV_FILENAME, TARGET_SENSOR_INDEX);
    
    let file = File::open(CSV_FILENAME).expect("GREŠKA: Nema vel.csv u folderu!");
    let reader = io::BufReader::new(file);

    // dohvat podataka iz određenog stupca
    let data: Vec<f32> = reader.lines()
        .filter_map(|line| line.ok()) // makni greške pri čitanju linije
        .filter_map(|line_str| {
            line_str.split(',')
                .nth(TARGET_SENSOR_INDEX) // uzimanje vrijednosti iz određenog stupca
                .map(|s| s.to_string())   // pretvorba u String kako bi se moglo parsirati
        })
        .filter_map(|s| s.trim().parse::<f32>().ok()) // parsiranje u float, ignoriraj greške
        .collect();

    if data.is_empty() {
        eprintln!("GREŠKA: Neuspjeh kod učitavanja mjerenja iz stupca {}.", TARGET_SENSOR_INDEX);
        return Ok(());
    }
